...
_hapi_path, _old_locals = _load_module(
    "hapi", _path.realpath(_path.join(_path.split(__file__)[0], _pardir)), locals()
)
...
-Lorentz-broadening note: hapi looks for n_self, and if it doesn't find that in the database,
assumes n_self = n_air for the line. If you want to hard-code a particular n_self = 0.694 or
similar, must add comma-separated value to Hitran database file.
...
def absorptionCoefficient_SDVoigt(
    ...
    # warn('To get the most up-to-date version of SDVoigt please check http://hitran.org/hapi.
    # This version is customized by Yang et al 2018')
    ...
    for Component in Components:
        ...
        if len(Component) >= 3:
            ...
            try:
                ni = ISO[(M, Idx)][ISO_INDEX["abundance"]]
            except KeyError:
                raise Exception("cannot find component M,Idx = %d,%d." % (M, Idx))
        ABUNDANCES[(M, Idx)] = ni
        NATURAL_ABUNDANCES[(M, Idx)] = ISO[(M, Idx)][ISO_INDEX["abundance"]]
        ...
        for RowID in range(nline):
            ...
            for species in Diluent:
                ...
                if power_law_shift is False:
                    ...
                    try:  # Check for n_delta parameter from local cache table
                        n_delta = LOCAL_TABLE_CACHE[TableName]["data"][
                            "n_delta_" + species_lower
                        ][RowID]
                    except:  # noqa: E722
                        n_delta = 1  # if it doesn't exist, default exponent to 1
                    # Apply power law scaling
                    Shift0 += abun * CustomEnvDependences.get(
                        delta_name, (Shift0DB * (Tref / T) ** n_delta * p)
                    )
                # End pld_update
...
# TODO: Do we want to make components and sourcetables required params?
    ...
    for TableName in SourceTables:
        ...
        for RowID in range(nline):
            ...
            for species in Diluent:
                ...
                # 20240226 SCC - Updating the code here to match the structure and
                # variables used in the SD Voigt calculation
                deltap_name = "deltap_" + species_lower
                deltap = 0.0
                try:
                    deltap = LOCAL_TABLE_CACHE[TableName]["data"][deltap_name][RowID]
                ...
                # Added this code to account for temperature dependent line
                # shifts when no deltap information is available
                ...
                if power_law_shift is False:
                    ...
                    try:  # Check the local cache table for n_delta
                        n_delta = LOCAL_TABLE_CACHE[TableName]["data"][
                            "n_delta_" + species_lower
                        ][RowID]
                    except:
                        n_delta = 1  # if it doesn't exist default to 1
                    # Apply power-law scaling
                    Shift0 += abun * CustomEnvDependences.get(
                        delta_name, (Shift0DB * (Tref / T) ** n_delta * p)
                    )
            ...
            BoundIndexUpper = bisect(Omegas, LineCenterDB + OmegaWingF)
            # 20240222 SCC:
            # The new version of hapi expects 5 positional arguments for
            # PROFILE_VOIGT() instead of 4; so I moved Shift0 to the 'Delta0'
            # positional argument for the updated function.
            # Also, I removed index [0] from above line because the new version
            # of hapi seems to already return the indexed value
