...
import numpy as np
import pandas as pd
import json #TODO: upgratde to json5
from .constants import HITRAN_160, MOLECULE_NAMES
...
def par_to_df(hit_file, nu_min=0, nu_max=10000):
    ...
    #        df_air['Smax'] = df_air['sw']*(.518*df_air['elower']/296)**(-2.77)
    # * np.exp(-1.435 * df_air['elower']*(1/(.518 * df_air['elower']) - 1/296))
    return df_air
...
def df_to_par(df, par_name, suffix="", extra_params={}, save_dir=None):
    ...
    ----------
    df: pandas DataFrame
        Dataframe containing HITRAN format database information
    par_name: str
        Name of the molecule - needs to match the HITRAN molecule list name
    suffix: str
        ??
    extra_params: dictionary
        ??
    save_dir: str
        File directory for saving .par file
    ...
    with open(os.path.join(save_dir, par_name + ".data"), "w") as out:
        for i in range(len(df)):
            ...
            for name in HITRAN_DEFAULT_HEADER["order"]:
                ...
                if "molec_id" in name:
                    value = MOLECULE_NAMES.index(par_name) + 1
                    ...
                    str_len = int(HITRAN_DEFAULT_HEADER["format"][name].split(
                        ".")[0][1:])
...
def calc_ierr(df_row, err_ref):
    ...
    for name, position in float_names.items():
        ...
        if uc > 0:
            ...
            err_ref[6 + 2 * position:6 + 2 *
                    (1 + position)] = "ps"  # Paul Schroeder fit_td
    return err_ref
...
def labfit_to_df(file_name, label_extension="", htp=False):
    ...
    titles_htp = [
        ...
        "uc_nu",
        "sw",
        "uc_sw",  # 'S296','uc_S296', # matches HITRAN_160 and LABFIT_EXTRA string names
        "elower",
        "uc_elower",
        "mass",
        "uc_mass",
    ...
    # I'm not 100% sure of the 10 params after n_delta_self
    ...
    #    print(linelist[0])
    #    print(other_params)
    #    print('linelist length', len(linelist[0]))

    # Now aggregate information into pandas dataframe
    df_linelist = pd.DataFrame.from_records(linelist, columns=titles_htp)
    # and add 4 string elements to front
    linelist_str_df = pd.DataFrame.from_records(linelist_str,
                                                columns=titles_str)
    df_linelist = pd.concat([linelist_str_df, df_linelist], axis=1)
    df_linelist = pd.DataFrame.sort_values(df_linelist, "index")
    # and make the dataframe indexing equivalent to Labfit indexing
    df_linelist = df_linelist.set_index("index")

    # First sort by amu (molecular mass),
    # Then other sort such as uc_elower
